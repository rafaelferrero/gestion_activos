...
@login_required
def getResumenGral(Request, cmd=''):
    resumen=[]
    registro=[]
    ...
    return render_to_response(plantilla, {'resumen':resumen},
...
def getResumenSeg(Request, cmd=''):
    ...
    for activo in activos:
        if activo.moneda=='$':
            tPesos = tPesos + float(activo.valor)
            ...
            tDolar=tDolar+float(activo.valor)
    ...
    return render_to_response(plantilla, {'activos':activos,'stPesos':stPesos,'stDolar':stDolar,
                                          'tPesos':tPesos,'tDolar':tDolar},
                                context_instance=RequestContext(Request))
...
def tmp(Request):
    ...
    return render_to_response(plantilla, {'activos':activos},
                                context_instance=RequestContext(Request))
